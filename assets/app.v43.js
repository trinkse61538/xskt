(() => {
  ...
  const ANOM = window.XSMN_ANOMALY || {stations:{}};
  ...
  function stationMatchOne(station,r){
    const st=stationHistory(station);
    if(!st || (st.draws||[]).length<100) return {station,available:false};

    const core=r.picks.slice(0,2);
    const pickWindows=core.map(num=>{
      const m10=numMetric(st,num,10),m30=numMetric(st,num,30),m100=numMetric(st,num,100);
      const weighted=.20*m10.rate+.35*m30.rate+.45*m100.rate;
      return {num,m10,m30,m100,weighted};
    });

    const r10=mean(pickWindows.map(x=>x.m10.rate));
    const r30=mean(pickWindows.map(x=>x.m30.rate));
    const r100=mean(pickWindows.map(x=>x.m100.rate));
    const raw=.20*r10+.35*r30+.45*r100;
    const spread=stdev([r10,r30,r100]);
    const adjusted=raw-.25*spread;

    return {
      station,available:true,st,
      core,pickWindows,r10,r30,r100,raw,spread,adjusted,
      minCore:Math.min(...pickWindows.map(x=>x.weighted))
    };
  }
  function stationMatchSummary(r){
    const baseline=HIST.baseline||0.165486;
    const ranked=stationsForDate(r.date)
      .map(s=>stationMatchOne(s,r))
      .filter(x=>x.available)
      .sort((a,b)=>b.adjusted-a.adjusted || b.raw-a.raw || a.station.localeCompare(b.station,'vi'));

    const best=ranked[0]||null, second=ranked[1]||null;
    const lead=(best&&second)?best.adjusted-second.adjusted:0;
    const clear=!!(best && second &&
      best.adjusted>=baseline+.02 &&
      lead>=.02 &&
      best.minCore>=baseline-.05);

    return {baseline,ranked,best,second,lead,clear};
  }
  function stationStability(x){
    if(!x)return '—';
    if(x.spread<=.025)return 'Ổn định';
    if(x.spread<=.06)return 'Khá ổn định';
    return 'Nhiễu';
  }
  function stationBlock(r) {
    const sm=stationMatchSummary(r);
    const stations=stationsForDate(r.date);
    if(!sm.ranked.length){
      return `<div class="station-panel caution">
        <div class="station-panel-head"><div><span>STATION MATCH · EXPERIMENTAL</span><b>Chưa đủ dữ liệu</b></div></div>
        <div class="station-note">Không đủ 100 kỳ lịch sử để tính Station Match.</div>
      </div>`;
    }
    const headline=sm.clear
      ? `Đài đối chiếu nổi bật: ${safe(sm.best.station)}`
      : 'Không có đài nổi bật';
    const headClass=sm.clear?'match-clear':'match-neutral';
    return `<div class="station-panel station-match-panel ${headClass}">
      <div class="station-panel-head">
        <div><span>STATION MATCH · EXPERIMENTAL · ${stations.length} đài</span><b>${headline}</b></div>
        <em>${sm.clear?'CLEAR MATCH':'NO CLEAR MATCH'}</em>
      </div>
      <div class="station-lock">Khóa số trước: <b>${safe(r.picks[0])}</b> · <b>${safe(r.picks[1])}</b> → sau đó mới so đài</div>
      ${sm.ranked.map((x,i)=>`<div class="station-row match-row ${i===0?'match-first':''}">
        <div class="station-rank">${i+1}</div>
        <div class="station-name">
          <b>${safe(x.station)}</b>
          <span>10/30/100: ${pct(x.r10)} · ${pct(x.r30)} · ${pct(x.r100)}</span>
          <small>Chủ 1 ${pct(x.pickWindows[0].weighted)} · Chủ 2 ${pct(x.pickWindows[1].weighted)} · ${stationStability(x)}</small>
        </div>
        <div class="station-score">
          <b>${pct(x.adjusted)}</b>
          <span>Match adj.</span>
          <small>raw ${pct(x.raw)}</small>
        </div>
      </div>`).join('')}
      <div class="station-rule">
        Match thô = 20%×10 kỳ + 35%×30 kỳ + 45%×100 kỳ, lấy trung bình Chủ 1 & Chủ 2.
        Điểm xếp hạng trừ 25% độ lệch giữa ba cửa sổ.
      </div>
      <div class="station-note">${sm.clear
        ? `Đài #1 vượt baseline ít nhất 2 điểm %, dẫn #2 ít nhất 2 điểm % và cả hai số chủ không quá yếu.`
        : `Chênh lệch chưa đủ ngưỡng để gọi một đài là nổi bật. Nếu vẫn tham khảo, hàng #1 chỉ là Match cao nhất chứ không phải “đài tốt hơn”.`}
      </div>
      <div class="station-foot">Backtest past-only 2006–2025 trên các ngày đạt điều kiện “clear”: +1.01 điểm % so với trung bình cùng ngày, p≈0.185; holdout 2023–2025 +3.16 điểm %, p≈0.083. Tín hiệu đáng theo dõi nhưng chưa xác nhận edge.</div>
    </div>`;
  }
  ...
  // ---------------- ANOMALY WATCH ----------------
  function anomalyStation(name){return ANOM.stations?.[name]||null}
  function anomalyNumber(station,num){const st=anomalyStation(station);return st?.numbers?.find(x=>x.n===String(num).padStart(2,'0'))||null}
  function anomalyLabel(a){
    if(!a)return {label:'Chưa có dữ liệu',cls:'anom-na',icon:'—'};
    if(a.state==='PERSISTENT_UP')return {label:'Bất thường tăng · bền',cls:'anom-pup',icon:'↑'};
    if(a.state==='PERSISTENT_DOWN')return {label:'Bất thường giảm · bền',cls:'anom-pdown',icon:'↓'};
    if(a.state==='WATCH_UP')return {label:'Theo dõi tăng',cls:'anom-wup',icon:'↑'};
    if(a.state==='WATCH_DOWN')return {label:'Theo dõi giảm',cls:'anom-wdown',icon:'↓'};
    return {label:'Bình thường',cls:'anom-normal',icon:'●'};
  }
  function anomalySeverity(a){if(!a)return 0;if(a.state.startsWith('PERSISTENT'))return 2;if(a.state.startsWith('WATCH'))return 1;return 0}
  function layerDecision(r){
    const sm=stationMatchSummary(r);
    if(!sm.clear||!sm.best)return {cls:'decision-neutral',title:'NEUTRAL · Chưa khóa được đài đối chiếu',note:'V5.1 đã khóa số nhưng Station Match chưa đủ rõ; Anomaly Watch hiển thị riêng từng đài.'};
    const st=sm.best.station,a1=anomalyNumber(st,r.picks[0]),a2=anomalyNumber(st,r.picks[1]),arr=[a1,a2].filter(Boolean);
    if(anomalyStation(st)?.stationDist?.anomaly)return {cls:'decision-station',title:'STATION WATCH · Phân phối toàn đài bất thường',note:`${st} vượt ngưỡng FDR ở kiểm định phân phối 00–99. Đây là cảnh báo integrity, không phải tín hiệu mua.`};
    if(arr.some(a=>anomalySeverity(a)>0&&a.dir==='DOWN'))return {cls:'decision-conflict',title:'CONFLICT · Các lớp đang mâu thuẫn',note:`Ít nhất một số chủ đang có lệch giảm tại ${st}. Không sửa số; chỉ ghi nhận xung đột dữ liệu.`};
    if(arr.some(a=>anomalySeverity(a)>0&&a.dir==='UP'))return {cls:'decision-align',title:'ALIGNMENT · Ba lớp cùng chiều',note:`Station Match rõ và ít nhất một số chủ có anomaly tăng tại ${st}. Đây là research alignment, không phải xác suất cao hơn.`};
    return {cls:'decision-neutral',title:'NEUTRAL · Không có anomaly đáng kể',note:`Station Match chọn ${st}, nhưng hai số chủ đang ở vùng thống kê bình thường.`};
  }
  function anomalyCoreTable(r,station){
    if(!anomalyStation(station))return `<div class="anom-empty">Chưa có dữ liệu Anomaly cho ${safe(station)}.</div>`;
    return r.picks.slice(0,2).map((num,i)=>{const a=anomalyNumber(station,num),l=anomalyLabel(a);return `<div class="anom-core-row ${l.cls}">
      <div class="anom-num"><span>${i?'Chủ 2':'Chủ 1'}</span><b>${safe(num)}</b></div>
      <div class="anom-state"><b>${l.icon} ${safe(l.label)}</b><span>Z30 ${a?.z30??'—'} · Z100 ${a?.z100??'—'} · Z300 ${a?.z300??'—'}</span><small>Drift Z ${a?.driftZ??'—'} · FDR ${a?.fdr?'CONFIRMED':'chưa xác nhận'}</small></div>
    </div>`}).join('');
  }
  function anomalyPanel(r){
    const sm=stationMatchSummary(r),d=layerDecision(r),preferred=sm.clear&&sm.best?sm.best.station:null,stations=stationsForDate(r.date);
    const stale=ANOM.updatedThrough&&HIST.updatedThrough&&ANOM.updatedThrough!==HIST.updatedThrough;
    return `<section class="three-layer-panel">
      <div class="layer-head"><span>3 LỚP ĐỘC LẬP</span><b>Không cộng điểm</b></div>
      <div class="layer-flow">
        <div class="layer-card l1"><span>LỚP 1</span><b>V5.1</b><strong>${safe(r.picks[0])} · ${safe(r.picks[1])}</strong><small>Khóa số</small></div>
        <div class="layer-arrow">→</div>
        <div class="layer-card l2"><span>LỚP 2</span><b>Station Match</b><strong>${preferred?safe(preferred):'Không rõ'}</strong><small>${sm.clear?'Clear match':'Không ép chọn'}</small></div>
        <div class="layer-arrow">→</div>
        <div class="layer-card l3"><span>LỚP 3</span><b>Anomaly Watch</b><strong>${preferred?'Kiểm tra':'Theo từng đài'}</strong><small>Chỉ cảnh báo</small></div>
      </div>
      <div class="layer-decision ${d.cls}"><b>${safe(d.title)}</b><span>${safe(d.note)}</span></div>
      ${stale?`<div class="anom-stale">⚠ Anomaly ${safe(ANOM.updatedThrough)} chưa khớp Recent Form ${safe(HIST.updatedThrough)}. Chạy workflow Update XSMN History một lần.</div>`:''}
      ${preferred?`<div class="anom-station-title">${safe(preferred)} · hai số đã khóa</div>${anomalyCoreTable(r,preferred)}`:
      `<div class="anom-multi">${stations.map(s=>`<div><b>${safe(s)}</b>${anomalyCoreTable(r,s)}</div>`).join('')}</div>`}
      <div class="anom-foot">Dùng 18 kết quả thô/kỳ, giữ số lặp; null 1% cho mỗi đuôi, cửa sổ 30/100/300 kỳ + FDR. Không tự suy ra lồng cầu/bóng bị lỗi nếu thiếu metadata thiết bị.</div>
    </section>`;
  }
  ...
  function renderCompare(){
    const r=closestRecord();
    const stations=stationsForDate(r.date);
    if($('stationMatchOverview')) $('stationMatchOverview').innerHTML=stationBlock(r)+anomalyPanel(r);
    ...
    $('compareWarning').innerHTML=`<b>Đây không phải xác suất kỳ tới.</b> Recent Form và Station Match chỉ được tính sau khi V5.1 đã khóa số. Chúng không sửa số, không cộng vào Điểm ngày và không chứng minh đài #1 có lợi thế xác suất.`;
  }
