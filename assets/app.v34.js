(() => {
  ...
  function stationRanking(iso) {
    ...
    const top=stations[0], second=stations[1];
    let verdict='Không có đài ưu tiên rõ ràng', tone='neutral',
        note='Các đài chỉ được xếp theo historical compatibility; không có predictive edge được xác nhận.';
    if(top) {
      const gap=second ? top.index-second.index : 0;
      if(top.index>=58 && top.trainTop3>0 && top.validation2025>0) {
        verdict=`Ưu tiên tham khảo: ${top.name}`; tone='strong';
        note='Top 3 excess hit-rate dương ở 2020–2024 và tiếp tục dương trong holdout 2025.';
      } else if(top.index>=52 && top.trainTop3>0 && top.validation2025>0 && gap>=2) {
        verdict=`Nghiêng về: ${top.name}`; tone='medium';
        note='Có tín hiệu lịch sử tương đối nhất quán, nhưng không đủ để xem là lợi thế chắc chắn.';
      } else if(top.index>=52 && gap>=3) {
        verdict=`Xếp #1 lịch sử: ${top.name}`; tone='caution';
        note='Đứng đầu nhóm theo lịch sử nhưng tín hiệu không tái lập ổn định trong 2025.';
      }
    }
  ...
  function stationBlock(r, compact=false) {
    ...
    return `<div class="station-panel ${sr.tone}">
      ...
      ${sr.stations.map((s,i)=>`<div class="station-row ${i===0?'top':''}">
        ...
        <div class="station-score"><b>${s.index}</b><span>Index</span></div>
      </div>`).join('')}
      <div class="station-note">${safe(sr.note)}</div>
      ${compact?'':`<div class="station-foot">Station Index dựa trên bộ 3 core V5.1 (Chủ 1 + Chủ 2 + Đảo/phụ) so với random baseline, 2020–2025. Index không phải xác suất.</div>`}
    </div>`;
  }
